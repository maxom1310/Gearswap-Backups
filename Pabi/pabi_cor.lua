...
--	but rather the specific roll like Chaos and such.  So rules needed instead for CorsairRoll above
	sets.precast["Double-Up"] = set_combine(sets.aftercast.Idle, {head="Lanun Tricorne +3",neck="Regal Necklace",hands="Chasseur's Gants +3",rring="Luzaf's Ring",back="Camulus's Mantle",})
	sets.precast["Quick Draw"] = {feet="Chass. Bottes +3"}--head="Laksa. Tricorne +3",}--"}
...
function self_command(command)
	...
	if command == 'psavage' then
		TP_Index = 1
		sets.aftercast.TP = sets.engaged[TP_Array[TP_Index]]
		status_change(player.status, player.status)
		send_command('@input /echo '..TP_Array[TP_Index]..' SET')
	end
		if command == 'pwildfire' then
		TP_Index = 2
		sets.aftercast.TP = sets.engaged[TP_Array[TP_Index]]
		status_change(player.status, player.status)
		send_command('@input /echo '..TP_Array[TP_Index]..' SET')
	end
		if command == 'pleaden' then
		TP_Index = 3
		sets.aftercast.TP = sets.engaged[TP_Array[TP_Index]]
		status_change(player.status, player.status)
		send_command('@input /echo '..TP_Array[TP_Index]..' SET')
	end
		if command == 'plow' then
		TP_Index = 4
		sets.aftercast.TP = sets.engaged[TP_Array[TP_Index]]
		status_change(player.status, player.status)
		send_command('@input /echo '..TP_Array[TP_Index]..' SET')
	end
		if command == 'pedge' then
		TP_Index = 5
		sets.aftercast.TP = sets.engaged[TP_Array[TP_Index]]
		status_change(player.status, player.status)
		send_command('@input /echo '..TP_Array[TP_Index]..' SET')
